(() => {
  ...
  function draw() {
    ...
    applyView();
    drawGrid(rect);
    for (const o of page().objects) paintObject(ctx, o);
  ...
  function eraseAt(pt) {
    ...
    const next = page().objects.filter((o) => !hitsObject(o, pt, radius));
  ...
  function setTool(tool) {
    ...
    canvas.style.cursor = tool === "pan" ? "grab" : tool === "laser" ? "none" : "crosshair";
  ...
  function finishText() {
    ...
    if (!text) return;
    if (textInput.dataset.mode === "note") {
      commit([
        ...page().objects,
